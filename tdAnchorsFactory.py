...
class TDAnchorsFactory:
    ...
    def _parse_anchors_rules(self, font: RFont, text: str):
        labels = {}
        glyphs = {}
        sfxlist = ['']
        shiftX = 0
        for line in text:
            line = self._clean_line(line).split('#')[0].strip()
            if line:
                if line.startswith('@'): # rule label
                    ...
                    if namelabel == '@SFXLIST':
                        ...
                        shiftX = int(content)
                        self.logger.info(f'ShiftX: {shiftX}')
                    else:
                        labels[namelabel] = content
                        self.logger.info(f'Label: {namelabel}, {content}')
                elif line.startswith('&'): # glyph unicode
                    glyphunicode = line.split('=')[0].replace('&', '')
                    umap = font.getCharacterMapping()
                    intunicode = int(glyphunicode, 16)
                    if intunicode in umap:
                        glyphname = umap[intunicode][0]
                        content = line.split('=')[1]
                        glyphs[glyphname] = content
                        self.logger.info(f'Glyph unicode: {glyphunicode}, {glyphname}, {content}')
                    else:
                        self.logger.error(f'Glyph unicode not found: {glyphunicode}, {line}')
                else: # glyph name
                    glyphname = line.split('=')[0]
                    content = line.split('=')[1]
                    glyphs[glyphname] = content
                    self.logger.info(f'Glyph name: {glyphname}, {content}')
    ...
    # def _getDecomposedGlyph(self, glyph: RGlyph):
    #     if glyph.components:
    #         dstGlyphL = RGlyph()
    #         dstGlyphL.width = glyph.width
    #         dstPenL = glyph.getPointPen()
    #         decomposePenL = DecomposingPointPen(glyphSet=glyph.font, dstPenL)
            
    #         glyph.drawPoints(decomposePenL)
    #         tempLeftGlyph = dstGlyphL
    #         return tempLeftGlyph
    #     else:
    #         return glyph


    def _get_right_margin_at_height(self, glyph: RGlyph, y_pos: int):
        def find_pointInside(glyph: RGlyph, y_pos: int):
            # _glyph = self._getDecomposedGlyph(glyph)
            if glyph.components:
                _glyph = glyph.font[glyph.components[0].baseGlyph]
            else:
                _glyph = glyph
            for x in range(xMax+10,xMin-10,-1):
                if _glyph.pointInside((x, y_pos)):
                    return (True, x)
            return (False, xMax)
        try:
        ...
        if y_pos == 0:
            y_pos = 1
            _, x = find_pointInside(glyph, y_pos)
            return x
        else:
            found, x = find_pointInside(glyph, y_pos - 1)
            if found:
                return x
            else:
                _, x = find_pointInside(glyph, y_pos + 1)
                return x
        return xMax
    ...
    def _get_topcenter_position(self, glyph: RGlyph):
        try:
            (xMin, yMin, xMax, yMax) = glyph.bounds
        except (AttributeError, TypeError):  # Catch specific exceptions when glyph.bounds is invalid
            self.logger.warning(f'Invalid bounds for glyph {glyph.name}. Using default values for topcenter.')
        xMin = self._get_left_margin_at_height(glyph, yMax-1)
        xMax = self._get_right_margin_at_height(glyph, yMax-1)
        return xMin + (xMax - xMin) / 2 #- self._calculate_italic_shift(glyph.font, yMax-yMin)
    
    def _get_bottomcenter_position(self, glyph: RGlyph):
        try:
            (xMin, yMin, xMax, yMax) = glyph.bounds
        except (AttributeError, TypeError):  # Catch specific exceptions when glyph.bounds is invalid
            self.logger.warning(f'Invalid bounds for glyph {glyph.name}. Using default values for bottomcenter.')
        xMin = self._get_left_margin_at_height(glyph, yMin+1)
        xMax = self._get_right_margin_at_height(glyph, yMin+1)
        return xMin + (xMax - xMin) / 2 #- self._calculate_italic_shift(glyph.font, yMax-yMin)
    ...
    def _calculate_anchor_position(self, font: RFont, glyph: RGlyph, align: str, y_pos: int):
        try:
            ...
            xMax = 0
        if y_pos != 0:
            xshift = self._calculate_italic_shift(font, y_pos)   
        else:
            xshift = 0
        if align == 'center':
            return glyph.width/2 + xshift
        elif align == 'left':
            if y_pos != 0:
                return xMin + xshift
            else:
            ...
            return self._get_topcenter_position(glyph)
        elif align == 'bottomcenter':
            return self._get_bottomcenter_position(glyph)
        elif align == 'centerpos':
            try:
                x1,y1,x2,y2 = glyph.bounds
                return (x2-x1)/2 + x1 #+ xshift
                    ...
                    if Ypos.startswith('$'):
                        ...
                        if '*' not in Ypos: # possible Ypos is a reference to another glyph
                            if not Ypos.endswith('_') and Ypos in font:
                                ...
                                if Ypos in font:
                                    try:
                                        (xMin, yMin, xMax, yMax) = font[Ypos].bounds
                                        Ypos = ((yMax - yMin) / 2) + yMin
                                    except (AttributeError, NameError):  # Catch errors when yMin is not accessible
                                        self.logger.error('ERROR 4 !!! Reference glyph not found:\n\t%s=%s' % (glyphname_, code))
                                        Ypos = 0
                                else:
                                    self.logger.error('ERROR 5 !!! Reference glyph not found:\n\t%s=%s' % (glyphname_, code))
                                    Ypos = 0
                        else:
                            baseg = Ypos.split('*')[0]
                            shiftY = Ypos.split('*')[1]
    ...
    def apply_anchors(self, font: RFont, anchors_code_list: list):
        labels, glyphs, sfxlist, shiftX = self._parse_anchors_rules(font, anchors_code_list)
        ...
        if self.UFOfile:
            try:
                ...
                if self.saveOutputUFOfile:
                    ...
                    font.save()
                    self.logger.info(f'Anchored font saved')
...
if __name__ == "__main__":
    ...
    parser.add_argument('--clear-anchors', action='store_true', help='Clear existing anchors', default=True)
    parser.add_argument('--replace-anchors', action='store_true', help='Replace existing anchors', default=True)
    parser.add_argument('--output', action='store_true', help='Save output UFO file')
    parser.add_argument('--save-existing', action='store_true', help='Save existing anchors', default=True)
